...
const rootPath = getCookie("root-path");
  ...
  return new Promise(async (resolve, reject) => {
    ...
    fetch(
      `${rootPath}/v1/lobby/${lobbyId}/gallery?` +
        new URLSearchParams({
          local_cache_count: cachedGallery ? cachedGallery.data.length : 0,
        }).toString(),
    )
      .then((response) => {
        if (response.status === 204) {
          console.log(`No new gallery data for lobby ${lobbyId} available.`);
        ...
        if (response.status === 200) {
          response
            ...
            .then((json) => {
              ...
              console.log(`Latest gallery for lobby ${lobbyId} stored.`);
              return json;
            })
            .then(resolve);
          return;
