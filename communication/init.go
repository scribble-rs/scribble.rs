...
func setupRoutes() {
	...
	http.Handle("/resources/", http.StripPrefix("/resources/", http.FileServer(pkger.Dir(pkger_workaround.Path("/resources/frontend")))))
...
func readTemplateFile(name string) string {
	templateHandle, pkgerError := pkger.Open(pkger_workaround.Path("/templates/" + name))
	if pkgerError != nil {
		panic(pkgerError)
	}
